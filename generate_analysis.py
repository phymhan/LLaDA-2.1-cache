...
Copy of generate_utils.py with generate_cached modified to record
confidence_history alongside decoding_order for AR-ness / confidence analysis.

Changes vs generate_utils.py:
  - generate_cached: decoding_order uses flat format (list of lists of ints),
    confidence_history added (parallel list of lists of floats).
  - Other functions (generate, generate_ssd_policy, helpers) unchanged.
"""
...
def generate_cached(
    ...
    **kwargs,
    ...
    Modified from generate_utils.py to record confidence_history alongside
    decoding_order for analysis purposes.

    When record_decoding_order=True, stats contains:
      - decoding_order: list of lists of ints (absolute positions unmasked per step)
      - confidence_history: list of lists of floats (x0_p values at those positions)
    ...
    Args:
        ...
        record_decoding_order: If True, record decoding_order and confidence_history
    ...
    Returns:
        ...
        When record_decoding_order is True, stats_dict also contains
        "decoding_order" and "confidence_history".
    ...
    confidence_history = [] if record_decoding_order else None
    ...
    for num_block in range(prefill_blocks, num_blocks):
        ...
        while True:
            ...
            # ── Record decoding order and confidence (flat format) ──
            if record_decoding_order:
                ...
                for pos in mask_transfer_index[0].nonzero(as_tuple=True)[0].tolist():
                    ...
                    step_positions.append(int(abs_pos))
                    step_confidences.append(float(x0_p[0, pos].item()))
                decoding_order.append(step_positions)
                confidence_history.append(step_confidences)
        ...
        # Check if we should stop before committing KV
        if eos_early_stop:
    ...
    if record_decoding_order:
        ...
        stats["confidence_history"] = confidence_history
    return result, stats
